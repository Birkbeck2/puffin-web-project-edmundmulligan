...
async function captureWebpage(url, outputName, options = {}) {
    ...
    if (mockStudentData) {
        info(`Mock student data: ${mockStudentData.name} (${mockStudentData.avatar}, ${mockStudentData.gender}, ${mockStudentData.age})`);
    }
    if (iterateSections !== null) {
        info(`Iterate sections: 0 to ${iterateSections - 1} (${iterateSections} total)`);
    }
    if (waitForSelector) info(`Wait for: ${waitForSelector}${waitForContent ? ' (with content)' : ''}`);
    ...
    try {
        ...
        page.on('console', msg => {
            ...
            } else if (type === 'log' && (text.includes('Theme') || text.includes('Style') || text.includes('Query'))) {
        ...
        const isHomepage = url.includes('localhost:8080?') || url.endsWith('localhost:8080') || url.includes('index.html');
        ...
        if (mockStudentData) {
            ...
            try {
                await page.evaluate((data) => {
                    ...
                    // Encrypt and save
                    return storage.encryptData(JSON.stringify(dataWithTimestamp))
                        .then(encryptedData => {
...
function main() {
    ...
    if (args.length < 2) {
        ...
        log('                              Mock student localStorage (avatar: wizard|witch, gender: male|female, age: young|old)');
    ...
    for (let i = 2; i < args.length; i++) {
        if (args[i] === '--width' && args[i + 1]) {
            ...
            i++;
        } else if (args[i] === '--mock-student-data' && args[i + 1] && args[i + 2] && args[i + 3] && args[i + 4]) {
