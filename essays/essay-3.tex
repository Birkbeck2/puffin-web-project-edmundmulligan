...
The development process was highly iterative, with continuous refinement driven by automated testing and manual code review. Given the scope of the project, and the fact that it was developed by a single person, this was the only feasible approach and drew on my considerable experience of developing in a DevOps environment. The website is at the limit of what can be achieved without server side processing. In particular, using local storage to persist data is inferior to using a database - if users switch computers their saved data will be lost and gamification, such as progress tracking and leader boards are not possible.

Although not the focus of this assignment, I am particularly please with how the CSS refactoring turned out. The CSS submitted last term was functional but messy, reflecting the fact that I was learning it while developing the project. The refactored version is much cleaner, particularly with the use of CSS variables in the definitions folder and layers. The Javascript code is reasonably well organised but could be improved. For example localStorage.js could and should, with more time, be refactored into classes to handle encryption and storage separately.
